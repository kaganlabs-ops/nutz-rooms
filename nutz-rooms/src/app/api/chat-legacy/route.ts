...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Load and inject relevant skills based on message
    await skillLoader.loadAll();
    const matchedSkills = skillLoader.getRelevant(message, 3);
    if (matchedSkills.length > 0) {
      systemPrompt += skillLoader.buildSkillPrompt(matchedSkills);
      console.log(`[CHAT] Injected ${matchedSkills.length} skills: ${matchedSkills.map(s => s.slug).join(', ')}`);
    }
